...
import { Plus, Trash2, Loader2, UploadCloud, Check } from "lucide-react";
...
import { updateReceiptItems, uploadReceiptImage, analyzeReceipt } from "@/lib/actions/receipts";

type ItemRow = { name: string; price: number; quantity: number };
...
export default function ReceiptDetailEditor({
  ...
  const [rows, setRows] = useState<ItemRow[]>(
    transaction.receiptItems.map(({ name, price, quantity }) => ({
      name,
      price,
      quantity,
    }))
  ...
  const [reanalyzePhase, setReanalyzePhase] = useState<ReanalyzePhase>("idle");
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const { receiptImageUrl } = await uploadReceiptImage(transaction.id, formData);
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      setRows(
        result.items.map(({ name, price, quantity }) => ({ name, price, quantity }))
      );
      setSaved(false);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="space-y-3 max-w-sm">
        ...
        <Button variant="outline" className="w-full" onClick={openReanalyze}>
          {currentImageUrl ? "別の画像で再解析する" : "画像をアップロードして解析する"}
        </Button>
      </div>
      ...
      <div className="space-y-3">
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow className="bg-muted/50 font-semibold">
                <TableCell colSpan={3} className="py-2 px-2 text-sm">
                  合計
                </TableCell>
        ...
        <div className="flex items-center gap-2 justify-end">
          ...
          <Button onClick={handleSave} disabled={saving} className="min-w-[96px]">
            {saving ? (
              <>
